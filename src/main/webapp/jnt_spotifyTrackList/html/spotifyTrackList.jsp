...
<c:forEach items="${currentNode.properties.track}" var="track" varStatus="stat">
	<c:set var="tracks" value="${tracks}${track.string}${stat.last ? '' : ','}" />
</c:forEach>
<c:if test="${not empty tracks}">
	<iframe src="https://embed.spotify.com/?uri=spotify:trackset:${title.string}:${tracks}&theme=${theme.string}&iew=${view.string}" width="${height.string}" height="${width.string}" frameborder="0" allowtransparency="true"></iframe>
</c:if>